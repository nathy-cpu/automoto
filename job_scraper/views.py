...
from .models import Job, CustomWebsite
from .scrapers import scrape_jobs, get_scraper
...
def job_search(request: HttpRequest):
    if request.method == 'POST':
        ...
        try:
            ...
            country = str(location) if location else 'us'  # Default to US if no location specified
            ...
            scraped_jobs = enhanced_scraper.get_recent_jobs(websites, country, search_keywords, max_pages=15)
...
def job_detail(request: HttpRequest, job_id: int):
    try:
        ...
        job_detail = {
            ...
            'city': job_data.get('location', '').split(',')[0] if job_data.get('location') else '',
            'country': 'United States',
            'description': job_data.get('description', f"This is a {job_data.get('title', 'job')} position at {job_data.get('company', 'a company')}. For more details, please visit the original job posting."),
            'requirements': job_data.get('requirements', '• Experience in the relevant field\n• Strong communication skills\n• Ability to work in a team environment\n• Relevant education or certifications'),
            ...
            'application_link': job_data.get('application_link', job_data.get('job_url', '')),
        ...
        return render(request, 'job_scraper/job_detail.html', {
            'job': None,
...
def manage_websites(request: HttpRequest):
    ...
    if request.method == 'POST':
        ...
        if all([name, base_url, search_url, job_list_selector, title_selector, company_selector, location_selector, job_link_selector]):
    ...
    custom_websites = CustomWebsite.objects.filter(is_active=True)
    return render(request, 'job_scraper/manage_websites.html', {
        'custom_websites': custom_websites
    })
