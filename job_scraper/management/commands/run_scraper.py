...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"Starting scraper for '{keywords}' in '{location}'..."))
        
        scraper = StealthScraper(headless=True)
        new_jobs = scraper.scrape_indeed(keywords, location, max_pages=1)

        self.stdout.write(self.style.SUCCESS(f"Found {len(new_jobs)} new jobs. Starting enrichment..."))
        ...
        for job in new_jobs[:limit]:
            try:
                ...
                self.stderr.write(self.style.ERROR(f"Failed to enrich {job.company}: {e}"))

        self.stdout.write(self.style.SUCCESS(f"Run complete. {len(new_jobs)} jobs processed, {enriched_count} contacts found."))
